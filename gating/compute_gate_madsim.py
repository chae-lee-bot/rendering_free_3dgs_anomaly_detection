"""MAD-Sim Hybrid gating + per-gaussian AUROC."""
import os
import numpy as np
...
DEN = f"{OUTPUT_ROOT}/madsim/eval_results/density_bidir.npz"
...
# 새 지표: density 신호 "집중도"
# 상위 5% density 신호가 전체 합에서 차지하는 비율 — 뭉쳐있으면 큼
concentration = np.array([
    np.sort(den_raw[i])[::-1][:max(1,len(den_raw[i])//20)].sum() / (den_raw[i].sum()+1e-9)
    for i in range(N)
])
# 또 하나: density 신호의 지니계수 비슷 — 상위10% / 중앙값
sharpness = np.array([
...
def report(label, score_list):
    ...
    tm = [np.mean(pt[a]) for a in ATYPES]
    allv = sum(pt.values(),[])
